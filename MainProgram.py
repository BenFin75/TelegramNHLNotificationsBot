...
todaysdb_win = PureWindowsPath('.\Database\\todaysgames.csv')
...
def helpcmd(update, context):
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id, text=help_msg, disable_web_page_preview=1)
...
def button(update, context: CallbackContext):
    ...
    if update.callback_query.data == '✔️':
        ...
        context.bot.send_message(
            chat_id=update.effective_chat.id, text=setup_msg)
    ...
    if update.callback_query.data == 'yes':
        ...
        updater.bot.sendMessage(
            chat_id=update.effective_chat.id, text="You will receive Notifications!")
    ...
    if update.callback_query.data == 'no':
        ...
        context.bot.deleteMessage(
            update.callback_query.message.chat.id, update.callback_query.message.message_id)
        updater.bot.sendMessage(
            chat_id=update.effective_chat.id, text="You will not receive Notifications!")
        databasemanagementnotifications(chat_id_noti, notification_pref)
    ...
    if update.callback_query.data == 'stay':
        ...
        context.bot.deleteMessage(
            update.callback_query.message.chat.id, update.callback_query.message.message_id)
        updater.bot.sendMessage(
            chat_id=update.effective_chat.id, text="Your team and notification data are safe")
...
def databasemanagementteams(formatted_team_ids, team_ids):
    ...
    if exists is False:
        newchatdf = pd.DataFrame({"ChatName": [chatname], "ChatID": [
                                 userid], "TeamIDs": [formatted_team_ids]})
...
def teamdatabasecheck(update, context, team_name):
    ...
    if team_name not in teamdf.TeamName.values:
        ...
        context.bot.send_message(
            chat_id=update.effective_chat.id, text=unknown_team_msg)
        return False
    else:
        return True


def postponedcheck(original_teams):
    playing_teams = []
    teams_list = original_teams.split(',')
    for team in teams_list:
        api_url = f'https://statsapi.web.nhl.com/api/v1/schedule?teamId={team}&date={todays_date}'
        r = requests.get(api_url)
        team_data = r.json()
...
def nextgame(update, context):
    ...
    if next_game_request.count(' ') == 0:
        context.bot.send_message(chat_id=update.effective_chat.id, text=(
            "Please indicate the team you want the next game for." + '\n' + "e.g. /nextgame Pens"))
        return
    ...
    if 'nextGameSchedule' not in elimcheck:
        ...
        teamid_name_form = str(
            formatted_teams_df.loc[formatted_teams_df.TeamID == teamid_next, 'TeamName'].values).strip("'[]")
        ...
        if away_team_score > home_team_score and away_team_id == teamid_next and playoff_check == 'P' or home_team_score > away_team_score and home_team_id == teamid_next and playoff_check == 'P':
            ...
            context.bot.send_message(
                chat_id=update.effective_chat.id, text=next_game_check_msg)
            return
        else:
            next_game_check_msg = ("Ha, The " + teamid_name_form +
                                   " have been eliminated from Stanley Cup Contention. Sucks to suck")
            context.bot.send_message(
                chat_id=update.effective_chat.id, text=next_game_check_msg)
            return
    ...
    if playoff_check == 'P':
        ...
        away_ot = json.dumps(next_game['teams'][0]['nextGameSchedule']
                             ['dates'][0]['games'][0]['teams']['away']['leagueRecord']['ot'])
        home_ot = json.dumps(next_game['teams'][0]['nextGameSchedule']
                             ['dates'][0]['games'][0]['teams']['home']['leagueRecord']['ot'])
...
def automation():
    ...
    chats_to_notify = list(
        chatsdf.loc[chatsdf['Notifications'] == 1, 'ChatID'])

    while len(chats_to_notify) != 0:
        userid = chats_to_notify[0]
        automaticgamenotification(userid, df)
        del chats_to_notify[0]
    timer()


def automaticgamenotification(userid, df):
    """
...
def gametimecsv(team_data, df):
    ...
    if team_data['totalItems'] >= 1:
        for i in team_data['dates'][0]['games']:
            ...
            current_index = df.index[-1]
            index = current_index + 1
        finaldf = df.drop_duplicates()
        finaldf.to_csv(todays_db, index=False, header=True)
    else:
        return
...
def last(update, context):
    ...
    if last_game_request.count(' ') == 0:
        context.bot.send_message(chat_id=update.effective_chat.id, text=(
            "Please indicate the team you want the last game for." + '\n' + "e.g. /lastgame Pens"))
        return
    ...
    teamid_last = int(
        lastdf.loc[lastdf.TeamName == team_name, 'TeamID'].values)
    ...
    updater.bot.sendMessage(
        chat_id=update.effective_chat.id, text=last_game_msg)
...
def status(update, context):
    ...
    team_names_df = pd.read_csv(teamsdb, encoding="ISO-8859-1")
    users_team_ids = list(
        chatsdf.loc[chatsdf['ChatID'] == chat_id_status, 'TeamIDs'].values)
    if users_team_ids == []:
        updater.bot.sendMessage(chat_id=update.effective_chat.id,
                                text="You have no data, please run /setup first!")
        return
    ...
    while len(users_team_ids_list) > 0:
        ...
        users_teams = formatted_teams_df.loc[formatted_teams_df['TeamID']
                                             == team, 'TeamName'].values
        teams_list = teams_list + users_teams + '?The '
        users_team_ids_list.remove(team)
    isusernotifi = chatsdf.loc[chatsdf['ChatID']
                               == chat_id_status, 'Notifications'].values
    if isusernotifi == 0:
    ...
    updater.bot.sendMessage(
        chat_id=update.effective_chat.id, text=status_message)
...
def roster(update, context):
    ...
    if roster_request.count(' ') == 0:
        context.bot.send_message(chat_id=update.effective_chat.id, text=(
            "Please indicate the team you want the roster of." + '\n' + "e.g. /roster Pens"))
        return
    ...
    updater.bot.sendMessage(chat_id=update.effective_chat.id,
                            text=f'<pre>{sorted_table}</pre>', parse_mode=ParseMode.HTML)
...
def player(update, context):
    ...
    if player_request.count(' ') == 0:
        context.bot.send_message(chat_id=update.effective_chat.id, text=(
            "Please indicate a team and player." + '\n' + "e.g. /player Pens Crosby"))
        return
    if player_request.count(' ') > 3:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text="One player at a time please.")
        return
    if player_request.count(' ') == 3:
    ...
    if found == 0:
        ...
        if player_info.isnumeric():
            ...
            player_msg = 'I cant find anyone named ' + \
                player_name_cap + ' on The ' + team_dec
...
def stats(update, context):
    ...
    if stats_request.count(' ') == 0:
        context.bot.send_message(chat_id=update.effective_chat.id, text=(
            "Please indicate a team and/or player." + '\n' + "e.g. /stats Pens " + " "*3 + "or" + " " * 3 + "/stats Pens Crosby"))
        return
    if stats_request.count(' ') > 3:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text="One player or team at a time please.")
        return
    if stats_request.count(' ') == 3:
        ...
        if found == 0:
            ...
            if player_info.isnumeric():
                ...
                player_msg = 'I cant find anyone named ' + \
                    player_name_cap + ' on The ' + team_dec
            updater.bot.sendMessage(
                chat_id=update.effective_chat.id, text=player_msg)
            return
    ...
    if teamstats == 1:
        ...
        games_played = json.dumps(
            team_data['stats'][0]['splits'][0]['stat']['gamesPlayed'])
        wins = json.dumps(team_data['stats'][0]['splits'][0]['stat']['wins'])
        losses = json.dumps(team_data['stats'][0]
                            ['splits'][0]['stat']['losses'])
        ot_losses = json.dumps(
            team_data['stats'][0]['splits'][0]['stat']['ot'])
...
def standings(update, context):
    ...
    if division_msg.count(' ') == 1:
        ...
        for n in standings['records']:
            ...
            if division_request == 'metro':
                division_request = 'metropolitan'
        ...
        if found == 0:
            ...
            updater.bot.sendMessage(
                chat_id=update.effective_chat.id, text=standings_msg)
            return
...
def testautonotify(update, context):
    ...
    if update.effective_chat.id == admin_chat_id:
        updater.bot.sendMessage(
            chat_id=update.effective_chat.id, text='Testing Automatic Notifications')
        automation()
        gametimecheck()
    else:
        return
...
def creategamelist(update, context):
    ...
    if update.effective_chat.id == admin_chat_id:
        updater.bot.sendMessage(chat_id=update.effective_chat.id,
                                text='Manually creating the game time csv for today!')
        df = pd.DataFrame(columns=('HomeIDs', 'AwayIDs', 'Time'))
        df.to_csv(todays_db, index=False, header=True)
        todays_date = date.today()
        chatsdf = pd.read_csv(chatdb)
